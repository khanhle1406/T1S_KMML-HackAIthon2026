...
* **Dockerfile**: Đóng gói môi trường CUDA 12.2 và vLLM offline cho Docker container.
* **requirements.txt**: Danh sách thư viện Python và các phiên bản tương thích được cài đặt trong container.
* **inference.sh**: Script khởi chạy entry-point chính của pipeline.
* **predict.py**: Script giải đề thi chính (thực hiện nạp vLLM, quét độ dài prompt, phân phối bất đồng bộ, dừng sớm theo độ phân vân Margin, biểu quyết số đông và ghi nhận thời gian thực thi tuần tự của từng câu hỏi).
* **tai_lieu_thuyet_minh.pdf**: Báo cáo thuyết minh giải pháp chính thức định dạng PDF, trình bày chi tiết về kiến trúc tối ưu hóa Logit Margin Early Exit và cơ chế chống tràn ngữ cảnh chủ động.
* **README.md**: Tài liệu hướng dẫn cài đặt, thực thi và mô tả luồng xử lý hệ thống.
